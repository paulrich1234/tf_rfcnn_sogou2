        ...
        if(t==0):
            ...
            for u in range(t):   
                ...
                if(cls=='cxd'):
                    sctext='bleeding'
                if(cls=='wdml'):
                    sctext='microaneurysm'
                if(cls=='jx'):
                    sctext='hematocele'
                ax.text(bbox[u][0], bbox[u][1] - 50,'{:s} {:.3f}'.format(sctext, score[u]),bbox=dict(facecolor='blue', alpha=0.5),fontsize=14, color='white',fontproperties=zhfont1)
            y=y+1
    if(y==0):
        ax.text(486+25, 60+25,'Health 0.997',bbox=dict(facecolor='blue', alpha=0.5),fontsize=14, color='white',fontproperties=zhfont1)
        ax.add_patch(plt.Rectangle((486, 60),2095-486,1665-60, fill=False,edgecolor='red', linewidth=3.5))
    else:
        #ax.text(486+25, 60+25,'Severe 0.999',bbox=dict(facecolor='blue', alpha=0.5),fontsize=14, color='white',fontproperties=zhfont1)
        #ax.add_patch(plt.Rectangle((485, 59),2176-485,1716-59, fill=False,edgecolor='red', linewidth=3.5))
        ax.text(2+25, 3+25,'Mild 0.999',bbox=dict(facecolor='blue', alpha=0.5),fontsize=14, color='white',fontproperties=zhfont1)
        ax.add_patch(plt.Rectangle((2, 3),1954-2,1932-3, fill=False,edgecolor='red', linewidth=3.5))

    ax.set_title(('Grading and lesion region detection: p(| box) >= {:.1f}').format(CONF_THRESH),fontsize=14,fontproperties=zhfont1)
    plt.axis('off')
    plt.tight_layout()
    plt.draw()
    #plt.savefig("/var/www/html/figure/"+image_name)
