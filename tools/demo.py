...
NETS = {'vgg16': ('vgg16_faster_rcnn_iter_70000.ckpt',),'res101': ('res101_faster_rcnn_iter_85000.ckpt',)}
...
def demo(sess, net, image_name):
    ...
    ax.imshow(im, aspect='equal')
    if os.path.exists('regions/'+image_name+'.csv'):
        os.remove('regions/'+image_name+'.csv')
    filer=open('regions/'+image_name+'.csv','a+')
    for cls_ind, cls in enumerate(CLASSES[1:]):
        ...
        if(t==0):
            ...
            for u in range(t):
                ...
                if(cls=='ai'):
                    sctext='9'
                filer.write(sctext+','+str(bbox[u][0])+','+str(bbox[u][1])+','+str(bbox[u][2])+','+str(bbox[u][3])+','+str(score[0])+'\n')
                ax.text(bbox[u][0], bbox[u][1] - 25,'{:s} {:.3f}'.format(sctext, score[u]),bbox=dict(facecolor='blue', alpha=0.5),fontsize=14, color='white')
            y=y+1
    if(y==0):
        ax.text(25, 25,'没有异常',bbox=dict(facecolor='blue', alpha=0.5),fontsize=14, color='white')

    ax.set_title(('result: p(| box) >= {:.1f}').format(t),fontsize=14)
    plt.axis('off')
    plt.tight_layout()
    plt.draw()
    #plt.savefig("/var/www/html/figure/"+image_name)
...
if __name__ == '__main__':
    ...
    #print(demonet)
    net.create_architecture(sess, "TEST", 37,
